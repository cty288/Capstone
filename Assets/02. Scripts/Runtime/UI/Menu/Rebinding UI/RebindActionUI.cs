...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        [SerializeField]
        private BindingDuplicateEvent m_bindingDuplicateEvent;
        public BindingDuplicateEvent bindingDuplicateEvent {
            get
            {
                if (m_bindingDuplicateEvent == null) 
                    m_bindingDuplicateEvent = new BindingDuplicateEvent();
                return m_bindingDuplicateEvent;
            }
        }
...
public InputActionRebindingExtensions.RebindingOperation ongoingRebind => m_RebindOperation;
        ...
        [SerializeField] private TMP_Text duplicationText;
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        if (CheckDuplicateBindings(action, bindingIndex, allCompositeParts)) {
                            action.RemoveBindingOverride(bindingIndex);
                            action.ApplyBindingOverride(bindingIndex, previousBindingPath);
                            UpdateBindingDisplay();
                            if (duplicationText) {
                            ...
                            CleanUp();
                            bindingDuplicateEvent?.Invoke(action, bindingIndex);
                            return;
                        }
            ...
            if (action.bindings[bindingIndex].isPartOfComposite)
                partName = Localization.GetFormat("MENU_SETTINGS_CONTROL_WAITING_BINDING_PART", action.bindings[bindingIndex].name);
            ...
            if (m_RebindText != null) {
                var text = Localization.GetFormat("MENU_SETTINGS_CONTROL_WAITING_BINDING", partName);
                m_RebindText.text = text;
            }

            // If we have no rebind overlay and no callback but we have a binding text label,
            // temporarily set the binding text label to "<Waiting>".
            if (!m_RebindOverlay  && !m_RebindText &&
                m_BindingText) {
                m_BindingText.text = Localization.Get("MENU_SETTINGS_CONTROL_WAITING_BINDING_2");
                bindingIcon.DOFade(0, 0);
                bindingText.DOFade(1, 0);
                //m_BindingText.gameObject.SetActive(true);
            }
        ...
        [SerializeField] private bool m_CanDuplicate = false;
        [SerializeField] private List<InputActionReference> duplicateWhitelist = new List<InputActionReference>();
        private bool CheckDuplicateBindings(InputAction action, int bindingIndex, bool allCompositeParts) {
            ...
            var allBindings =
                duplicateWhitelist.Select((reference => reference.ToInputAction().bindings)).ToList();

            HashSet<string> allActions = new HashSet<string>();

            foreach (var bindingArray in allBindings) {
                foreach (InputBinding binding in bindingArray) {
                    allActions.Add(binding.action);
                }
            }
            ...
            foreach (var binding in action.actionMap.bindings) {
                ...
                if (binding.effectivePath == newBinding.effectivePath) {
                    if (allActions.Contains(binding.action)) {
                        continue;
                    }
        ...
        {
            var imageGO = m_BindingText.transform.parent.Find("ActionBindingIcon");
            if (imageGO) {
                bindingIcon = imageGO.GetComponent<Image>();
            }
        ...
        [Serializable]
        public class BindingDuplicateEvent : UnityEvent<InputAction, int> {
