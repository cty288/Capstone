...
namespace Runtime.BehaviorDesigner.Composite
{
    [TaskDescription("Like RandomSelect except only selects moves for the current stage.")]
    [TaskIcon("{SkinColor}RandomSelectorIcon.png")]
    public class StageBaseSelector : global::BehaviorDesigner.Runtime.Tasks.Composite
    {
        public SharedInt currentStage;
        public List<string> includedTasksPerStage;

        [global::BehaviorDesigner.Runtime.Tasks.Tooltip("Seed the random number generator to make things easier to debug")]
        public int seed = 0;
        [global::BehaviorDesigner.Runtime.Tasks.Tooltip("Do we want to use the seed?")]
        ...
        public override void OnStart()
        {
            ...
            if (currentStage.Value < includedTasksPerStage.Count)
                // if the current stage is less than the number of stages, use the current stage's list of attacks
                childIndexList = includedTasksPerStage[currentStage.Value].Split(',').Select(int.Parse).ToList();
            else
                // otherwise, use the last stage's list of attacks
                childIndexList = includedTasksPerStage[includedTasksPerStage.Count - 1].Split(',').Select(int.Parse).ToList();
