...
namespace _02._Scripts.Runtime.WeaponParts.Model.Instance.ShortBarrel {
	...
	public class ShortBarrelBuff : WeaponPartsBuff<ShortBarrel, ShortBarrelBuff> {
		...
		private HitData OnModifyHitData(HitData data, IWeaponEntity weapon) {
			...
			Transform attacker = data.Attacker?.GetRootDamageDealerTransform();
			...
			return new List<GetResourcePropertyDescriptionGetter>() {
				new GetResourcePropertyDescriptionGetter(() => {
					...
					return new WeaponBuffedAdditionalPropertyDescription(iconName, title,
						Localization.GetFormat("ShortBarrel_desc", displayMultiplayer, displayDistance));
				})
			};
		}
	}
