...
public class CharacterViewer : MonoBehaviour {
	...
	void Update () {
        ...
        {
            targetForCamera.position=new Vector3(targetForCamera.position.x,targetForCamera.position.y+((Input.mousePosition - lastPosition).y / Screen.height)*500* Time.unscaledDeltaTime,targetForCamera.position.z);
	...
	void LateUpdate () {
     ...
     if (  !Input.GetMouseButton(1))    cameras.rotation = rotation;
    }
}
