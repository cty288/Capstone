...
namespace SCPE
{
    ...
    public class PostEffectRenderer<T> : ScriptableRenderPass
    {
        ...
        private void CreateMaterialIfNull(ref Material material, ref Shader m_shader, string m_shaderName)
        {
            ...
            //Shaders are referenced on the corresponding volume profile asset, so this operation will always succeed in a build.
            if(!m_shader) m_shader = Shader.Find(m_shaderName);
             
#if UNITY_EDITOR || DEVELOPMENT_BUILD
            if (!m_shader)
            {
                Debug.LogError("[SC Post Effects] Shader with the name <i>" + m_shaderName + "</i> could not be found, this means it was not included in the build. This will happen when effects are entirely initialized through scripting. Be sure to reference the shader somewhere.");
                return;
            }
#endif
        ...
        /// </summary>
        protected void SetViewProjectionMatrix(CommandBuffer cmd, in CameraData cameraData)
        {
